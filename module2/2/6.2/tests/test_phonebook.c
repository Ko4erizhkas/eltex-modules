/*
 * Юнит-тесты для libphonebook (задание 6.2)
 *
 * Тест собирается ровно так же, как и сама программа: подключает только
 * заголовок библиотеки и линкуется с разделяемой libphonebook.dll (.so).
 * Поэтому он проверяет не только логику списка, но и саму сборку библиотеки:
 * при отсутствии экспорта тест не слинкуется, а при отсутствии самой
 * библиотеки рядом с exe - не запустится.
 *
